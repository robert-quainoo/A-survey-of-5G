...
ALPHA = 1.25  # Alpha value for EIED
BETA = 0.8  # Beta value for EIED
...
class Node:
    ...
    def transmit_data(self):
        ...
        for neighbor in self.neighbors:
            ...
            self.retries = 0
            self.contention_window = max(int(self.contention_window * BETA), CW_MIN)  # EIED decrease
            self.cw_history.append(self.contention_window)  # Record contention window
            self.cw_timestamps.append(time.time())  # Record timestamp
        self.transmitting = False

    def backoff(self):
        backoff_slots = random.randint(0, self.contention_window)
        ...
        self.contention_window = min(int(self.contention_window * ALPHA), CW_MAX)  # EIED increase
        self.cw_history.append(self.contention_window)  # Record contention window
        self.cw_timestamps.append(time.time())  # Record timestamp
